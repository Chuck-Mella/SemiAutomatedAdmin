...
(
    ...
    [string]$rptPath = '\\logserver\share\SystemReports'
)
...
Function Get-SystemReport
{
    ...
    Version    : 5.0.2.5 20240402 Current Build ( Added central data collection functions to enable remote collection )
    #>
    [CmdletBinding()]
    Param (
    $CurrentTime,
    ...
    function Get-DataCollection($system,$srchMethod)
    {
        ...
        $results = [Ordered]@{
        OSInfo = $(Try { Get-CimInstance -Class Win32_OperatingSystem -ComputerName $system -EA Stop }
        ...
        DiskInfo = $(Try { Get-CimInstance -Class Win32_LogicalDisk -ComputerName $system -EA Stop }
        ...
        aclShares = $null
        Services = $(Try { Get-WmiObject -Class Win32_Service -ComputerName $system -EA Stop | Where {$_.Startmode -eq 'Auto' -and $_.State -eq 'Stopped'} }
        Catch { Get-Service -ComputerName $system | Where {$_.StartType -eq 'Automatic' -and $_.Status -EQ 'Stopped'} })
        evtSys = $(Try { Get-EventLog -ComputerName $system -LogName System -EntryType Error,Warning -Newest $evtCount -EA Stop }
        Catch { $_ })
        evtApp = $(Try { Get-EventLog -ComputerName $system -LogName Application -EntryType Error,Warning -Newest $evtCount -EA Stop }
        Catch { $_ })
        evtSec = $(
        ...
        Apps = $(
            ...
            {
                $Apps = Get-Package -IncludeSystemComponent -IncludeWindowsInstaller -AllVersions -ProviderName
                $Apps.count
                $Apps[0].Metadata.Keys | Select *
                $Apps[0] | Select Name,Version,FullPath,FromTrustedSource,FastPackageReference,IsCorpus,IsPatch,IsSupplemental,Links,Meta,Metadata,PackageFilename,Payload,ProviderName,SearchKey,Source,Status,Summary,SwidTags,SwidTagText,TagId,TagVersion,VersionScheme
                ($Apps[0].Metadata).GetHashCode() 
                $Apps |GM
            }
            }
        $Apps)
        rcntSW = $($rs=Try { Get-WinEvent -ComputerName $system -ProviderName msiinstaller -EA Stop | where id -eq 1033 | Sort Timecreated -desc }
        Catch { $_;$rs })
        ...
        HV_SWs = $(Try{If ((Get-Service vmms -ComputerName $System -ea Stop).Status -eq 'Running'){ Get-VMSwitch }}Catch{$_}) # | Select Name,SwitchType,@{n='NetAdapter';e={$_.NetAdapterInterfaceDescription}} | ConvertTo-Csv -NoTypeInformation)}
        HV_VMs = $(Try{If ((Get-Service vmms -ComputerName $System -ea Stop).Status -eq 'Running'){ Get-VM }}Catch{$_}) # | Select * | OGV)
        VS_Code = $(Try{Code --? -ea Stop}Catch{$_})
        ...
        If ($results.VS_Code.psObject.Properties.Name -notcontains 'Exception'){ $results.VS_Code = (code --list-extensions | % { "code --install-extension $_" }) }
    ...
    ForEach ($System in $Systems)
    {
        ...
        If ($sysData.Count -ne 16){ Write-Host -f ye "Some data appears to be missing" }
        ...
        $TopProcesses = $sysData.TopProc | Sort WS -Descending | Select ProcessName, Id, WS -First $ProccessNumToFetch
        ...
        If ($sysData.rcntSW.psObject.Properties.Name -contains 'Exception' -or $sysData.rcntSW -eq $null)
        {
        ...
        Else
        {
            $recentSoftwareReport = $sysData.rcntSW | where id -eq 1033 | Sort Timecreated -desc | 
