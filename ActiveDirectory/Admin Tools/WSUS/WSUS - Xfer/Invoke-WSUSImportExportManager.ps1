<#
    .SYNOPSIS
        The purpose of this script is to handle the export/import process for WSUS
        without internet connectivity. WSUS with Internet connectivity will use this 
        script to perform an EXPORT to get: WSUS Content, WSUS Metadata, WSUS Config-
        uration XML file WSUS without internet connectivity will use this script to 
        perform an IMPORT and get same updates, WSUS Configuration and update approvals 
        as the internet connected WSUS


    .DESCRIPTION
        The script can be used to export and import the configuration, metadata and 
        content from a WSUS server. A simple metadata export does not include which 
        updates were approved or not. By using this script, we can export this information
        too and "sync" which updates are approved to what computer groups in WSUS.
        If you want a change to persist on your disconnected WSUS instances, you will
        have to make the changes on the internet-connected WSUS server where we perform
        the export.
#>
