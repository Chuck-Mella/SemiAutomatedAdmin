<#
    .SYNOPSIS
        ...
        WSUS with Internet connectivity will use this script to perform an EXPORT to get: WSUS Content, WSUS Metadata,
        WSUS Configuration XML file WSUS without internet connectivity will use this script to perform an IMPORT and
        get same updates, WSUS Configuration and update approvals as the internet connected WSUS

    .DESCRIPTION
        The script can be used to export and import the configuration, metadata and content from a WSUS server.
        A simple metadata export does not include which updates were approved or not. By using this script, we can export
        this information too and "sync" which updates are approved to what computer groups in WSUS. If you want a change
        to persist on your disconnected WSUS instances, you will have to make the changes on the internet-connected WSUS
        server where we perform the export.
#>
...
'@
    ...
    Function Invoke-WSUSDBReindex
    {
        <#
            ...
            .LINK
               https://gallery.technet.microsoft.com/scriptcenter/Invoke-WSUSDBMaintenance-af2a3a79 
        #>
        Param
        (
        )
...
"@
    ...
    Function Add-ChildComputerGroupToParentXML
    {
        ...
        If($ComputerTargetGroup.Id -eq 'b73ca6ed-5727-47f3-84de-015e03f6a88a')
        {  #  Handle Unassigned Computers group for different languages
        ...
        {  #  Current group has childs, recurse
            ...
            if($IncludeApprovals)
            {
                ...
                Get-WSUSUpdates -WSUSServer $WSUSServer | Where-Object {$_.IsApproved -eq $true} | ForEach-Object
                {
                    ...
                    foreach($approval in $approvals)
                    {
                        ...
                        If($($approval.Deadline).Year -ne '9999')
                        {  #  Assuming no deadline if year is 9999
    ...
    Function New-ComputerGroupFromXML
    {
        ...
        if($NewComputerGroupElement)
        {  #  Create group and then recurse to create child groups
            $groupName = $newComputerGroupElement.'#text'
            if($groupName -notin $allGroups)
            {  #  Group does not exists on target WSUS at this time.
                if(!$currentchildGroupNames)
                {  #  Parent Group has no child, create the group
            ...
            {  #  Group with same computer name already exists
                [bool]$NeedsToBeDeleted = $false
                if(!$currentchildGroupNames)
                {  #  Parent Group has no child, delete the existing computer group with the same name
        ...
        elseif($NewComputerGroupName)
        {  #  Create group only
            $groupName = $NewComputerGroupName
            if($groupName -notin $allGroups)
            {  #  Group does not exists on target WSUS at this time, create the group
                Add-TextToCMLog $LogFile  "Creating group named `"$($groupName)`" as a child group of `"$($ParentComputerGroup.Name)`"." $component 1
    ...
    Function Import-WSUSConfigurationFromXML
    {
        ...
        if(($currentDownloadExpressPackages -ne $downloadExpressPackages) -OR 
            ($currentDownloadUpdateBinariesAsNeeded -ne $downloadUpdateBinariesAsNeeded) -OR 
            ($currentHostBinariesOnMicrosoftUpdate -ne $false) -OR ($currentGetContentFromMU -ne $false))
        {
        ...
        if($languages)
        {
            ...
            if(!$currentLanguages -or $diffs)
            { # There are differences between languagues currently used on the WSUS Server and the XML configuration file
        ...
        if(($WSUSConfiguration.AutoApproveWsusInfrastructureUpdates -ne $false) -or 
            ($WSUSConfiguration.AutoRefreshUpdateApprovals -ne $false))
        {
        ...
        if(!$WSUSConfigOnly)
        {  #  Do not continue if WSUSConfigOnly is true
            ...
            if($computerGroups)
            {
                ...
                if($($allComputersGroupElement.'#text') -eq $allComputersGroupName)
                {
                    ...
                    {
                        if($subGroup -is [System.Xml.XmlElement])
                        {  #  Computer group has sub groups
                            New-ComputerGroupFromXML -ParentComputerGroup $allComputersGroup -NewComputerGroupElement $subGroup
                        }
                        elseif($subGroup -is [String])
                        {  #  Computer Group does not have any child groups
                            If($subGroup -ne $unassignedComputersGroupName)
                            {  #  Ignore 'Unassigned Computers' since it's built-in
    ...
    function Start-ConsoleProcess
    {
        <#
            ...
            .LINK
                https://github.com/RamblingCookieMonster/PSDeploy/tree/master/PSDeploy/Private 
        #>
        Param
        (
            [Parameter(Mandatory = $true)]
    ...
    function Get-WSUSUtil
    {
        ...
        {}
        if(!$Wsusutil)
        {  #  WsusUtil not in the default path, let's try finding WsusUtil.exe from the log file path in WSUS Configuration
            Try
            {
...
#region Parameter validation
    ...
    if($ParentPath -eq "")
    {  #  Filename only, using script location
    ...
    Try
    {
        ...
        [bool]$Import = (($Actions | Where-Object {$_.Name -eq "Import"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
        If($Import)
        {
            $ImportOptions = ($Actions | Where-Object {$_.Name -eq "Import"}).Option
            [string]$SourceDir = $ImportOptions | Where-Object {$_.Name -eq "SourceDir"} | 
                Select-Object -ExpandProperty Location
            [bool]$IsWSUSContentSelected = (($ImportOptions | Where-Object {$_.Name -eq "WSUSContent"} | 
                Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
            [bool]$IsMetadataSelected = (($ImportOptions | Where-Object {$_.Name -eq "Metadata"} | 
                Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
            if($IsMetadataSelected)
            {
                [string]$MetadataFilename = $ImportOptions | Where-Object {$_.Name -eq "Metadata"} | 
                    Select-Object -ExpandProperty Filename
                [bool]$DropSUSDB = (($ImportOptions | Where-Object {$_.Name -eq "Metadata"} | 
                    Select-Object -ExpandProperty DropSUSDB) -eq [bool]::TrueString)
            }
            [bool]$IsXMLConfigSelected = (($ImportOptions | Where-Object {$_.Name -eq "XMLConfiguration"} | 
                Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
            if($IsXMLConfigSelected)
            {
                [string]$XMLConfigFileName = $ImportOptions | Where-Object {$_.Name -eq "XMLConfiguration"} | 
                    Select-Object -ExpandProperty Filename
                [bool]$IncludeApprovals = (($ImportOptions | Where-Object {$_.Name -eq "XMLConfiguration"} | 
                    Select-Object -ExpandProperty IncludeApprovals) -eq [bool]::TrueString)
            }
        }
        [bool]$Export = (($Actions | Where-Object {$_.Name -eq "Export"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
        If($Export)
        {
            $ExportOptions = ($Actions | Where-Object {$_.Name -eq "Export"}).Option
            [string]$DestinationDir = $ExportOptions | Where-Object {$_.Name -eq "DestinationDir"} | 
                Select-Object -ExpandProperty Location
            [bool]$IsWSUSContentSelected = (($ExportOptions | Where-Object {$_.Name -eq "WSUSContent"} | 
                Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
            [bool]$IsMetadataSelected = (($ExportOptions | Where-Object {$_.Name -eq "Metadata"} | 
                Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
            if($IsMetadataSelected)
            {
                [string]$MetadataFilename = $ExportOptions | Where-Object {$_.Name -eq "Metadata"} | 
                    Select-Object -ExpandProperty Filename
            }
            [bool]$IsXMLConfigSelected = (($ExportOptions | Where-Object {$_.Name -eq "XMLConfiguration"} | 
                Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
            if($IsXMLConfigSelected)
            {
                [string]$XMLConfigFileName = $ExportOptions | Where-Object {$_.Name -eq "XMLConfiguration"} | 
                    Select-Object -ExpandProperty Filename
                [bool]$IncludeApprovals = (($ExportOptions | Where-Object {$_.Name -eq "XMLConfiguration"} | 
                    Select-Object -ExpandProperty IncludeApprovals) -eq [bool]::TrueString)
            }
        }
        [bool]$ReindexSUSDB = (($Actions | Where-Object {$_.Name -eq "ReindexSUSDB"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
        [bool]$UseCustomIndexes = (($Actions | Where-Object {$_.Name -eq "UseCustomIndexes"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
        [bool]$RemoveCustomIndexes = (($Actions | Where-Object {$_.Name -eq "RemoveCustomIndexes"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)

        [bool]$SetIISWSUSPoolSettings = (($Actions | Where-Object {$_.Name -eq "SetIISWSUSPoolSettings"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
        ...
        [bool]$ShowLocallyPublishedUpdates = (($Actions | Where-Object {$_.Name -eq "ShowLocallyPublishedUpdates"} | 
            Select-Object -ExpandProperty Enabled) -eq [bool]::TrueString)
    ...
    If (!$UseCustomIndexes -and !$RemoveCustomIndexes -and 
        !$Export -and !$Import -and !$ReindexSUSDB -and 
        !$SetIISWSUSPoolSettings -and !$ShowLocallyPublishedUpdates)
    {
...
if($Import -or $Export)
{
    Try
    {
        ...
        $CurrentWSUSContentDir = ((Get-Item ($WSUSServer.GetConfiguration() | 
            Select-Object -ExpandProperty LocalContentCachePath)).Parent).FullName
...
Add-TextToCMLog $LogFile ("#"*93) $component 1
